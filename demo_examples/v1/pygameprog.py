...
while 1:
    ...
    for e in pygame.event.get():
        if e.type == QUIT:
            ...
            selected_obj +=1
            if selected_obj >= len(objs):
                selected_obj = 0
            oo = objs[selected_obj]
            
        elif e.type == MOUSEBUTTONDOWN:
            if e.button == 4: zpos = max(1, zpos-1)
            elif e.button == 5: zpos += 1
            elif e.button == 1: rotate = True; mouse(mouse_pos[0], mouse_pos[1])
            elif e.button == 3: move = True
        elif e.type == MOUSEBUTTONUP:
            if e.button == 1: rotate = False; #update_rotate_vals(oo)
            elif e.button == 3: move = False; #update_move_vals(oo)
        elif e.type == MOUSEMOTION:
            i, j = e.rel
            if rotate:
                oo.rx = float(oo.rx) + float(i)
                oo.ry = float(oo.ry) + float(j)
            if move:
                oo.tx = float(oo.tx) + float(i)
                oo.ty = float(oo.ty) - float(j)
        ...
        if e.type == pygame.MOUSEBUTTONUP and e.button == 1:
            ...
            glEnable(GL_DEPTH_TEST)
            storeit = map.mouseLeftClick(e.pos[0], e.pos[1])
